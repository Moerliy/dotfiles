...
I recommend using all the dotfiles together but it is possible to just use hyprland with some additional personal changes.

** 1. Clone
If you don't have existing dofiles you can clone my hole repository (for that you should have a look at my README in my dotfile directory) or just hyprland:
...
+ [[https://github.com/freedesktop/polkit][polkit]] authentication agent (gnome version)
...
+ [[https://github.com/flatpak/xdg-desktop-portal][xdg-desktop-portal]] frontend service for Flatpak and possibly other desktop containment frameworks
...
$term = alacritty
$menu = $HOME/.cargo/bin/anyrun     # not working
...
** Not Working
#+begin_src conf
bind = $mainMod, R, exec, $menu     # not working
bind = $mainMod, Y, exec, $music    # not working
